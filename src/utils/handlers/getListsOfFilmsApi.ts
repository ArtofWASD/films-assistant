const getListsOfFilmsApi = async (type:string) => {
    const item = await fetch(
      `https://api.kinopoisk.dev/v1.3/movie?selectFields=name&selectFields=nameEn&selectFields=alternativeName&selectFields=shortDescription&selectFields=rating.kp&selectFields=rating.imdb&selectFields=movieLength&selectFields=poster.previewUrl&selectFields=genres.name&selectFields=countries.name&selectFields=status&selectFields=year&selectFields=id&page=1&limit=10&type=${type}&year=2023`,
      {
        method: 'GET',
        headers: {
